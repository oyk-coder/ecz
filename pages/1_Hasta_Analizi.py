...
with st.sidebar:
    ...
    st.session_state.show_ddi_heatmap = st.toggle("DDI Isı Haritası", value=st.session_state.show_ddi_heatmap)
    st.session_state.show_cars_radar  = st.toggle("CARS Bileşen Grafiği", value=st.session_state.show_cars_radar)
    st.session_state.show_fall_chart  = st.toggle("Düşme Risk Grafiği", value=st.session_state.show_fall_chart)
    ...
    with st.expander("Hasta Formunu Aç"):
        f_ad       = st.text_input("Ad Soyad")
        f_yas      = st.number_input("Yaş", min_value=18, max_value=110, value=70)
        ...
        f_charlson = st.number_input("Charlson İndeksi", min_value=0, max_value=37, value=0)
        f_morse    = st.number_input("Morse Düşme Skoru", min_value=0, max_value=125, value=20)
        f_tanilar  = st.text_input("Tanılar (virgülle ayır)", value="")
        f_alerjiler= st.text_input("Alerjiler (virgülle ayır)", value="")
        ...
        if st.button("Kaydet ve Ekle"):
            if not f_ad.strip():
                ...
                try:
                    ...
                    st.success(f_ad.strip() + " başarıyla eklendi!")
                    st.rerun()
...
st.error(
    ...
    "Gerçek klinik kararlarda eczacı ve hekim kontrolü şarttır."
)
...
with col_card:
    ...
    st.markdown(f"ID: {patient.hasta_id} | Yaş: {patient.yas} | Cinsiyet: {patient.cinsiyet}")
    ...
    st.markdown("**Tanılar:** " + ", ".join(patient.kronik_hastaliklar) if patient.kronik_hastaliklar else "Tanı yok")
...
with col_metrics:
    ...
    with m1:
        st.metric("CARS Skoru", cars["skor"], help="0-100 arası kompozit risk skoru")
    with m2:
    ...
    with m3:
        ...
        st.metric("DDI Etkileşim", len(ddi_hits), delta=f"{crit_ddi} kritik" if crit_ddi else None, delta_color="inverse")
    with m4:
        st.metric("PDFI Skoru", fall_risk["skor"], help="Düşme risk skoru")
 
    poly = " - Polifarmasi tespit edildi (5+ ilaç)!" if len(aktif_ilaclar) >= 5 else ""
    st.caption(
        f"{len(aktif_ilaclar)} aktif ilaç analiz edildi.{poly} "
...
with st.expander("Reçete Girişi - Yeni İlaç Ekle"):
    col_i1, col_i2, col_i3 = st.columns(3)
    with col_i1:
        sec_ilac = st.selectbox("İlaç Seç", [il["isim"] for il in ilac_listesi_mock])
    with col_i2:
        sec_doz = st.text_input("Doz (örnek: 5mg)", value="")
    with col_i3:
        sec_frek = st.selectbox("Frekans", frekanslar)
 
    if st.button("Ekle ve Analiz Et"):
        if not sec_doz.strip():
            st.warning("Doz alanı boş bırakılamaz.")
        else:
...
if aktif_ilaclar:
    st.markdown("**Aktif İlaç Listesi**")
...
with viz2:
    ...
    if st.session_state.show_ddi_heatmap and aktif_ilaclar:
        ...
        st.caption("Kırmızı hücre: KRİTİK DDI - eczacı müdahalesi zorunlu.")
    ...
    for hit in pim_f:
        with st.expander(f"{hit['ilac']} - {hit['kategori']} | {hit['siddet']}"):
            ...
            with ca:
                ...
                st.markdown(f"**Şiddet:** {hit['siddet']}")
...
with tab_ddi:
    ...
    for rule in ddi_f:
        ...
        with st.expander(f"{rule['ilac_1_adi']} x {rule['ilac_2_adi']} | {rule['siddet']}", expanded=is_crit):
            cx, cy = st.columns(2)
            with cx:
                st.markdown(f"**Şiddet:** {rule['siddet']}")
...
with tab_drugs:
    if not aktif_ilaclar:
        st.info("Bu hasta için henüz ilaç eklenmemiştir.")
    icons = {FallRiskClass.CRITICAL:"Kritik", FallRiskClass.HIGH:"Yüksek",
             FallRiskClass.MODERATE:"Orta", FallRiskClass.LOW:"Düşük"}
    for drug in aktif_ilaclar:
        st.markdown(
            f"**{drug.isim}** | Doz: `{drug.doz}` | Frekans: `{drug.frekans}` | "
            f"ATC: `{drug.atc}` | Düşme Riski: {icons.get(drug.fall_risk_class, '?')}"
        )
...
st.caption("CARS algoritması + Beers/STOPP kural motoru. RAG-Grounded Inference.")
...
with b3:
    if st.button("Prescriber'ı Ara", use_container_width=True):
        ...
        st.session_state.decision_log.append({
            "zaman": datetime.now().strftime("%d.%m.%Y %H:%M:%S"),
            "hasta": patient.ad_soyad, "karar": "HEKİM ARANACAK", "cars_skor": cars["skor"],
        })
with b4:
...
decision_map = {
    ...
    "HEKIM_ARAND": ("warning", "Hekim acil bildirim protokolu başlatıldı."),
...
if not cohort_df.empty:
    ...
    fig_s = px.scatter(
        ...
        color="Risk Seviyesi",
        color_discrete_map={"KRİTİK":"#ef5350","YÜKSEK":"#ffa726","ORTA":"#ffee58","DÜŞÜK":"#66bb6a"},
        hover_name="Hasta",
...
st.caption("PharmaSentinel-RX v1.0.0 - Eğitim Prototipi")
